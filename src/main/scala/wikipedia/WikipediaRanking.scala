...
object WikipediaRanking {
  ...
  val sc: SparkContext =
    new SparkContext("local[4]", "WikipediaRankingApp", conf)
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd
      ...
      .aggregate(0)((qtde, article) => qtde + 1, _ + _)
  }
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    langs
      ...
      .sortBy(tuple => tuple._2)(Ordering[Int].reverse)

  /* Compute an inverted index of the set of articles, mapping each language
   * to the Wikipedia pages in which it occurs.
   */
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    val step2: WikipediaArticle => List[(String, WikipediaArticle)] =
      article => {
        langs.flatMap(
          lang =>
    ...
    val step1: RDD[(String, WikipediaArticle)] = rdd.flatMap(step2)
    val result: RDD[(String, Iterable[WikipediaArticle])] = step1.groupByKey()
    result
  }
...
//    index
//      .map(pair => (pair._1, pair._2.count(wp => wp.mentionsLanguage(pair._1))))
//      .sortBy(tuple => tuple._2, ascending = false)
//      .collect()
//      .toList

    index
      .mapValues(iterable => iterable.size)
      .sortBy(tuple => tuple._2, ascending = false)
      .collect()
      .toList
  }
